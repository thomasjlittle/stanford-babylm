...
def get_all_hidden_states(
    ...
    with tqdm(total=len(input_list), disable=tqdm_disable) as progress_bar:
        while st < len(input_list):
            ...
            inputs, len_mask, input_lens = tokenizer_helper(cslice)
            ...
            if input_masks is not None:
                ...
                mask = t_shaped_mask(tree_mask, len_mask, num_layers)
            ...
            for idx, _ in enumerate(cslice):
                ...
                if sum_all:
                    hidden_states = [hs.sum(axis=0) for hs in hidden_states]
    ...
    if sum_all:
        ...
        return get_word_vecs_from_subwords(input_list, hidden_states_all, tokenizer, pre_tokenized)
...
def get_word_vecs_from_subwords(input_list, hidden_states_all, tokenizer, pre_tokenized=None):
    def get_cumulants(hs, idx_list):
        all_vecs = []
        for st, en in idx_list:
            curr_vec = hs[st:en].sum(axis=0)  # sum up all the subword token representations!
...
def measure_sim_factory(distance_fn):
    def measure_sim(m1, m2):
        if m1.ndim == 2:
            ...
            return sum(measure_sim(m1[idx], m2[idx]) for idx in range(len(m1))) / (1.0 * len(m1))
...
def get_pre_tokenized_info(tokenizer, input_str):
    ...
    words = [
        word if idx == 0 else " {}".format(word) for idx, word in enumerate(input_str.split(" "))
    ]
    idxs = []
    # go in order.
    st = 0
...
class TreeProjection:
    ...
    def _mask_all_possible(self, input_str):
        ...
        """
        # weird gpt2 tokenization
        all_tokens = [
            word if idx == 0 else " {}".format(word)
            for idx, word in enumerate(input_str.split(" "))
        ]
        tokenized_inp = self.tokenizer(input_str)["input_ids"]
        word_tokenized = [
            self.tokenizer(word, add_special_tokens=False)["input_ids"] for word in all_tokens
        ]
            ...
            if is_leaf_fn is not None and is_leaf_fn(word_list, st, en):
                ...
                if randomize:
                    ...
                    for k in range(st, en):
                        if is_invalid_fn is not None and is_invalid_fn(word_list, st, k, en):
                            continue
                ...
                if normalize:
                    ...
                    rand_val = chart_values[(st, rand_split)] + chart_values[(rand_split + 1, en)]
                    best_val -= rand_val
    ...
    def __call__(
        ...
        sentence2idx_tuple, masked_strs, input_masks = self._get_masking_info([input_str])

        outer_context_vecs = get_all_hidden_states(
            self.model,
            self.tokenizer,
            [input_str],
            tqdm_disable=True,
        ...
        inner_context_vecs = get_all_hidden_states(
            self.model,
            self.tokenizer,
            masked_strs,
            input_masks,
...
#     model.to(device)
#     projector = TreeProjection(model, tokenizer)
#     projector("The man likes apples", st_threshold=0)
